...
Runs a single Jest pass for the last analyzed files:
- candidate -> manual tests + ai_generated_tests/**

Then reports failed tests and coverage deltas for each analyzed file,
comparing against the pre-existing coverage report (generated by the
last `covai analyze` / `covai run` pass).
"""
...
class VerificationRunner:
    ...
    def run(self, target_file: Optional[str] = None) -> dict:
        ...
        # Load prior coverage once from the existing report, before running
        # any new Jest pass, so we have a stable "before" baseline.
        prior_coverage = self._load_prior_coverage()
            ...
            for file_path in files:
                all_results.append(
                    VerificationFileResult(
                        file_path=file_path,
                        before_coverage=prior_coverage.get(file_path),
    ...
    def _load_prior_coverage(self) -> dict[str, float]:
        """
        Load coverage percentages from the pre-existing lcov report that was
        produced by the last `covai analyze` / `covai run` pass.

        Searches the standard coverage output directories for both frontend
        and backend projects. Falls back to an empty dict if no report is
        found, in which case `before_coverage` will show as n/a.
        """
        # Candidate locations for the pre-existing coverage report.
        # Adjust these paths to match wherever your Jest configs write their
        # coverageDirectory during a normal (non-verification) run.
        candidate_paths = [
            self.project_root / "coverage" / "lcov.info",
            self.project_root / "coverage" / "backend" / "lcov.info",
            self.project_root / "coverage" / "frontend" / "lcov.info",
            self.project_root / "client" / "coverage" / "lcov.info",
        ]

        combined: dict[str, float] = {}
        for lcov_path in candidate_paths:
            if lcov_path.exists():
                combined.update(self._read_coverage_map(lcov_path))
